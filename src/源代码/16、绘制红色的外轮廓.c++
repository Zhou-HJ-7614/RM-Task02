...
int main(int argc, char** argv) {
    ...
    // 在原图上绘制红色轮廓
    cv::drawContours(image, contours, -1, cv::Scalar(0, 0, 255), 2); // 红色轮廓

    // 保存绘制后的图像到原目录
    std::string outputPath = imagePath.substr(0, imagePath.find_last_of('/')) + "/output_image_with_contours.jpg";
